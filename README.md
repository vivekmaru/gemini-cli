...
If you have cloned the repository and made changes, you can run your modified
version without overwriting the global installation by using a shell alias.
...
2.  **Create an alias (e.g., `modgemini`):**
    ...
    Run the following command from the repository root to add the alias to your
    shell configuration (replace `~/.zshrc` with `~/.bashrc` if using Bash):
